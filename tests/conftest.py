...
def set_config(request):
    ...
    request.config.stash['telegram-notifier-addfields']['report'] = 'https://supervlad52.github.io/dm_api_tests/'
...
def mailhog_api():
    mailhog_configuration = MaihogConfiguration(host=v.get('service.mailhog'), disable_log=False)
...
def auth_account_helper(mailhog_api):
    dm_api_configuration = DmApiConfiguration(host=v.get('service.dm_api_account'), disable_log=False)
    account = DMApiAccount(configuration=dm_api_configuration)
    account_helper = AccountHelper(mailhog=mailhog_api, dm_account_api=account)
